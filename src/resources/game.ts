...
export function registerGame(
  ...
  const g = program.command('game').description('Games API: supported games and deck categorization rules');
      ...
      printData(
        ...
        (color) => {
          ...
          return tableFromRows(
            ['id', 'name', 'formats', 'platforms', 'metagame'],
            rows,
            color,
          );
        },
...
`,
    ...
    .action(async (id: string) => {
      ...
      printData(
        ...
        data,
        (color) => {
          const rows = data.map((d) => [d.identifier, d.name, String((d.variants?.length ?? 0) + (d.cards?.length ? 1 : 0))]);
          return tableFromRows(
            ['id', 'name', 'rules~'],
            rows,
            color,
